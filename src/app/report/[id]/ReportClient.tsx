    ...
    <div className="min-h-screen pb-20">
      ...
      <div className="max-w-4xl mx-auto px-4 pt-8">
        ...
        <div className="mt-12 mb-8">
          <ShareButtons score={report.overallScore} url={report.url} reportId={id} />
        </div>
        <div className="text-center"><Link href="/" className="text-orange-500 hover:underline text-sm">Roast another page →</Link></div>
      </div>

      {showPayment && <PaymentModal reportId={id} walletAddress={WALLET} price={PRICE} onSuccess={() => { setShowPayment(false); fetchReport() }} onClose={() => setShowPayment(false)} />}
    </div>
  )
}
